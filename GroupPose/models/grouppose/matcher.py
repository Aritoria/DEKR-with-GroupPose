...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # print("tgt_arer",tgt_area.shape,tgt_area) # tgt_arer torch.Size([3]) tensor([0.3033, 0.0830, 0.4830], device='cuda:0')
        ...
        # print("cost_class", cost_class.shape, cost_class) # cost_class torch.Size([300, 3]) tensor([[0.3087, 0.3087, 0.3087],
        ...
        if Z_pred.sum() > 0:
            ...
            cost_oks = 1 - oks
            # print("cost_oks", cost_oks.shape, cost_oks) # torch.Size([300, 3]) tensor([[0.8812, 1.0000, 0.9997],
            ...
            # print("cost_keypoints", cost_keypoints.shape, cost_keypoints) # torch.Size([300, 3]) tensor([[ 5.1758,  7.4018,  2.2278],

            C = self.cost_class * cost_class + self.cost_keypoints * cost_keypoints + self.cost_oks * cost_oks
            # print("C", C.shape, C) # torch.Size([300, 3]) tensor([[ 55.8998,  78.6358,  26.8940],
            C = C.view(bs, num_queries, -1).cpu() # torch.Size([3,100, 3])
        ...
        # print("sizes", torch.Tensor(sizes).shape, sizes) # sizes torch.Size([3]) [1, 1, 1]
        # 最后一维分割C
        indices = [linear_sum_assignment(c[i]) for i, c in enumerate(C.split(sizes, -1))]
        # print("indices", torch.Tensor(indices).shape, indices)

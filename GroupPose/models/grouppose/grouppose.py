...
class GroupPose(nn.Module):
    ...
    def __init__(self, backbone,
                 ...
                 num_queries,  # 100
                 aux_loss=False,
                 num_feature_levels=1,  # 4
                 nheads=8,
                 two_stage_type='no',  # 'standard'
                 dec_pred_class_embed_share=False,  # False
                 dec_pred_pose_embed_share=False,  # False
                 two_stage_class_embed_share=True,  # False
                 two_stage_bbox_embed_share=True,  # False
                 cls_no_bias=False,  # False
                 num_body_points=17
                 ):
        super().__init__()
        ...
        self.detr = PoseHigherResolutionNet()
        self.detr.init_weights()
        ...
        # prepare class
        # 如果不包含偏置（cls_no_bias为True），则直接定义了一个没有偏置的线性层。
        ...
        if dec_pred_class_embed_share: # 该share是层级share的意思
        ...
        # 在主流程中和decoder中的class和pose分数预测共享
        self.class_embed = nn.ModuleList(class_embed_layerlist) # 为100个选中的query在层级中进行分类预测监督
        self.pose_embed = nn.ModuleList(pose_embed_layerlist) # 根据点的对特征预测一个xy便宜量
        ...
        if two_stage_bbox_embed_share:
            ...
            self.transformer.enc_pose_embed = copy.deepcopy(_keypoint_embed)  # 256 -> 512 -> 34

        if two_stage_class_embed_share:
            self.transformer.enc_out_class_embed = _class_embed
        else:
            self.transformer.enc_out_class_embed = copy.deepcopy(_class_embed)  # 256 -> 2
    ...
    def forward(self, samples: NestedTensor):
        ...
        with torch.no_grad():
            # pass
            # print(image.shape)
            heatmap, offset, dekr_poses, dekr_scores = self.detr(image)
            # print(dekr_poses.shape)
            # heatmap, offset = self.detr(image)
            # print(heatmap.shape)
            # print(offset.shape)
        ...
        hs_pose, refpoint_pose, mix_refpoint, mix_embedding = self.transformer(srcs, masks, poss, dekr_poses)
...
def build_grouppose(args):
    ...
    if args.two_stage_type != 'no':
        ...
        interm_weight_dict.update(
            {k + f'_interm': v * args.interm_loss_coef * _coeff_weight_dict[k] for k, v in clean_weight_dict.items()})
        weight_dict.update(interm_weight_dict)

    losses = ['labels', "keypoints", "matching"]

    criterion = SetCriterion(num_classes, matcher=matcher, weight_dict=weight_dict, focal_alpha=args.focal_alpha,
                             losses=losses, num_body_points=args.num_body_points)
    criterion.to(device)
    postprocessors = {

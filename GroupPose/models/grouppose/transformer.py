...
# from .hrnet_dekr import PoseHigherResolutionNet
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=256,
                 ...
                 enc_n_points=4,
                 dec_n_points=4,
                 learnable_tgt_init=False,  # False
                 two_stage_type='no',
                 num_body_points=17
                 ):
        super().__init__()
    ...
    def forward(self, srcs, masks, pos_embeds, dekr_poses):
        """
        Input:
        ...
        # print(dekr_scores,dekr_scores.shape)
        # print(dekr_poses,dekr_poses.shape)
        src_flatten = []
        mask_flatten = []
        ...
        if self.two_stage_type in ['standard']:
            ...
            # 将output_memory通过一个256，256的线性层和一个归一化层
            output_memory = self.enc_output_norm(self.enc_output(output_memory))
            # print("111",output_memory.shape)
            # top-k select index
            topk = self.num_queries

            enc_outputs_class_unselected = self.enc_out_class_embed(
                output_memory)  # 256->2 对每一个特征生成两个类别 和 主流程中的不是一个类别预测

            topk_proposals = torch.topk(enc_outputs_class_unselected.max(-1)[0], topk, dim=1)[
                1]  # topk_proposals -> torch.Size([4, 100])
            #
            for i in range(len(dekr_poses)):
                H_2 = spatial_shapes[1, 0]
                W_2 = spatial_shapes[1, 1]
                dekr_pose = dekr_poses[i]
                if len(dekr_pose) > 0:
                    dekr_pose = dekr_pose / 512.0

                    dekr_pose_without_score = dekr_pose[..., 0:2]
                    dekr_pose_center = torch.mean(dekr_pose_without_score, dim=2, keepdim=True)
                    dekr_pose_center_fl = dekr_pose_center.flatten(-2)
                    skip_num = level_start_index[1]

                    index = torch.mean(dekr_pose_center_fl, dim=-1, keepdim=True)
                    index.fill_(skip_num)
                    H_2_tensor = torch.empty_like(index)
                    H_2_tensor.fill_(H_2)
                    W_2_tensor = torch.empty_like(index)
                    W_2_tensor.fill_(W_2)
                    index = index + W_2_tensor * dekr_pose_center_fl[:, :, 0].unsqueeze_(-1) + W_2_tensor * (
                                (H_2_tensor * dekr_pose_center_fl[:, :, 1].unsqueeze_(-1)) - 1)
                    index = torch.floor(index).flatten(-2)
                    # 替换dekr的检测人体
                    topk_proposals[i, -index.size(-1):] = index
                # print("index", index.shape, index)
            #

            # print("222",topk_proposals.shape,topk_proposals)
            # calculate K, e.g., 17 for COCO, points for keypoint
            bs, nq = output_memory.shape[:2]
            # self.enc_pose_embed 没有和其他共享
            delta_unsig_keypoint = self.enc_pose_embed(output_memory).reshape(bs, nq, -1,
                                                                              2)  # 256 -> 512 -> 34 对每一个feature中的向量进行34个点的预测
            enc_outputs_pose_coord_unselected = (delta_unsig_keypoint + output_proposals[..., :2].unsqueeze(
                -2)).sigmoid()  # 34个点 加上 proposals 的坐标后进行sigmoid # torch.Size([4, feat_num, 17, 2])
            enc_outputs_center_coord_unselected = torch.mean(enc_outputs_pose_coord_unselected, dim=2,
                                                             keepdim=True)  # 求中心点 # torch.Size([4, feat_num, 1, 2])
            enc_outputs_pose_coord_unselected = torch.cat(
                [enc_outputs_center_coord_unselected, enc_outputs_pose_coord_unselected], dim=2).flatten(
                -2)  # torch.Size([4, feat_num, 36])
            # gather pose
            enc_outputs_pose_coord_sigmoid = torch.gather(enc_outputs_pose_coord_unselected, 1,
                                                          topk_proposals.unsqueeze(-1).repeat(1, 1,
                                                                                              enc_outputs_pose_coord_unselected.shape[
                                                                                                  -1]))  # 从top100中拿出来那100个
            refpoint_pose_sigmoid = enc_outputs_pose_coord_sigmoid.detach()  # 参考sigmoid
            for i in range(len(dekr_poses)):
                dekr_pose = dekr_poses[i]
                if len(dekr_pose) > 0:
                    dekr_pose = dekr_pose / 512.0
                    ...
                    # print("dekr_pose_without_score", dekr_pose_without_score.shape)
                    # print("dekr_pose_center", dekr_pose_center.shape)
                    dekr_pose_all = torch.cat([dekr_pose_center,dekr_pose_without_score],dim=2).flatten(-2)
                    # print("dekr_pose_all", dekr_pose_all.shape,dekr_pose_all)
                    # 加噪声
                    noise = torch.randn_like(dekr_pose_all) * 0.025
                    # 加和
                    refpoint_pose_sigmoid[i, -dekr_pose_all.size(-2):] = dekr_pose_all + noise
            refpoint_pose_sigmoid = refpoint_pose_sigmoid.detach()

            # print("refpoint_pose_sigmoid", refpoint_pose_sigmoid)
            # gather tgt
            ...
            if self.learnable_tgt_init:
                ...
                tgt = tgt_undetach.detach().unsqueeze(-2)  # tgt torch.Size([4, 100, 1, 256])
                # print("tgt",tgt.shape)
            # query construction
            ...
            # dekr_poses 是一个4元列表，每一个列表中包含一个图像中解码出来的人

            # print(len(dekr_poses))
            # # dekr_poses, dekr_scores
        ...
        hs_pose, refpoint_pose = self.decoder(
            ...
            spatial_shapes=spatial_shapes,
            valid_ratios=valid_ratios)
        # print("refpoint_pose",refpoint_pose)
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            refpoint_pose_reshape = refpoint_pose_input[:, :, 0].reshape(nq, bs, np // 2, 2).reshape(nq * bs, np // 2,
                                                                                                     2)
            pose_query_sine_embed = gen_sineembed_for_position(refpoint_pose_reshape).reshape(nq, bs, np // 2,
                                                                                              self.d_model)
